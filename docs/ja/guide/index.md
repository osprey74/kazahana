...
>
> **独自 PDS 対応:** kazahana はセルフホスト PDS（Personal Data Server）上のアカウントでもログインできます。通常通りハンドルを入力するだけで、DNS／well-known および DID Document を経由して対応する PDS エンドポイントを自動的に解決します。PDS URL を手動で入力する欄はありません。なお、メールアドレスでのログインは `bsky.social` アカウントのみ対応のため、独自 PDS のユーザーはハンドルでログインしてください。
...
| **画像を追加** | 画像ファイルを選択して添付します（最大10枚、自動圧縮 2MB以下／旧仕様 PDS には 1MB フォールバック）。5 枚以上添付すると自動的に `app.bsky.embed.gallery` で投稿され、タイムラインでは横スクロールカルーセル + 枚数バッジで表示されます |
| **動画を追加** | 動画ファイルを選択して添付します（最大 300MB、サーバ側でトランスコード） |
