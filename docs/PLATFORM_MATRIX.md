...
> - Desktop (Windows / Tauri build): https://github.com/osprey74/kazahana
...
> **macOS 提供形態:** 旧 Tauri 版 macOS ビルドは廃止済みのため、本マトリクスから「macOS (Tauri)」列を削除しました（2026-09-05）。macOS は Mac App Store で配信する **macOS (Catalyst)** 版に一本化されています。Windows は引き続き Tauri デスクトップビルドで提供します。

> **⚠️ 更新スコープの厳守:** 各プラットフォームの Claude Code は、**自分のリポジトリが担当する列のみ**を更新すること。
> - Desktop リポジトリ（kazahana）の Claude Code → **Windows** 列のみ
> - kazahana-ios の Claude Code → **iOS** 列 および **macOS (Catalyst)** 列
> - kazahana-android の Claude Code → **Android** 列のみ
>
> 担当外プラットフォームのセルは、ソースを直接確認できない限り変更しないこと。実装状況を確認せずに他列の記号（✅ / ⬜ / 🚧）を書き換えることを禁止する。不明な場合は `❓`（要確認）のままにし、担当リポジトリの Claude Code による確認を待つ。
...
> **Windows** は Tauri デスクトップビルドで提供されます（本マトリクスの唯一の Tauri 列）。
...
| 動画再生（HLS） | ✅ | ✅ | ✅ | ✅ | |
| リンクカード（OGP） | ✅ | ✅ | ✅ | ✅ | 文字コード自動判定: Desktop v3.4.2 / iOS v3.5.0 / Android v3.5.0 で Content-Type / `<meta charset>` を優先する HTML living standard 準拠の検出に対応（Shift_JIS / EUC-JP 等の非 UTF-8 サイト文字化け解消）。iOS は `CFStringConvertIANACharSetNameToEncoding`、Android は `Charset.forName` で全 IANA charset 名に対応 |
...
| 下書き保存（最大20件） | ✅ | ✅ | ✅ | ✅ | |
| キーボードショートカット（Ctrl+N / Ctrl+Return / Ctrl+R） | ✅ | ✅ | N/A | N/A | Windows は Ctrl 系。Catalyst は Cmd+N / Cmd+Return / Cmd+R |
...
| プロフィール内検索 | ✅ | ✅ | ✅ | ✅ | |
| **プロフィール QR コード生成・共有** | N/A | N/A | ✅ | ✅ | Bluesky v1.124 同梱機能。自分のプロフィールから QR シートを開き `bsky.app/profile/{handle}` を符号化。リンクのコピー / 共有 / QR 画像のギャラリー保存。iOS / Android のみ（Desktop・macOS Catalyst はスコープ外）。HANDOFF_kazahana-profile-qr.md。Android v3.4.0：ZXing で生成、`MediaStore` 保存。iOS v3.4.0：CIFilter.qrCodeGenerator で生成 |
...
| 会話一覧 | ✅ | ✅ | ✅ | ✅ | |
| メッセージ送受信 | ✅ | ✅ | ✅ | ✅ | |
| **チャット内メッセージ返信表示（受信、`replyTo`）** | ✅ | ✅ | ✅ | ✅ | Bluesky social-app v1.125.0（2026-06-17 リリース）対応。`chat.bsky.convo.defs#replyRef` / `messageView.replyTo` 受信 → 返信元バブルプレビュー＋削除済み (`deletedMessageView`) フォールバック表示。返信元タップで元バブルへ平滑スクロール + フラッシュ。Desktop v3.5.0 / iOS v3.5.0 / Android v3.5.0 |
| **チャット内メッセージ返信送信（`messageInput.replyTo`）** | ✅ | ✅ | ✅ | ✅ | コンテキストメニュー（Android は長押し）から「返信」→ composer 上部に返信先チップ（キャンセル可）→ `sendMessage` で `replyTo: { messageId }` 付与。`ReplyTargetNotFound` をローカライズ表示。Desktop v3.5.0 / iOS v3.5.0 / Android v3.5.0 |
| 絵文字リアクション | ✅ | ✅ | ✅ | ✅ | |
| 新規会話作成 | ✅ | ✅ | ✅ | ✅ | |
| メッセージ削除 | ✅ | ✅ | ✅ | ✅ | |
| ミュート / 退出 | ✅ | ✅ | ✅ | ✅ | |
...
5. **更新スコープの厳守**: 各 Claude Code は自リポジトリ担当列のみ更新する（Desktop→Windows / kazahana-ios→iOS・macOS (Catalyst) / kazahana-android→Android）。担当外の列はソース確認なしに書き換えず、不明時は `❓` のままにする
6. **将来のリネーム候補**: macOS は Catalyst 版に一本化済み。将来 UI 上の表記を「macOS (Catalyst)」から「macOS」へ簡略化する場合は、全表のヘッダーを一括更新する（現時点では Catalyst 表記を維持）
