...
This release brings full client-side support for Bluesky's group chat feature, which the official client launched in v1.124.0 (released 2026-06-10). All four implementation phases from the cross-platform handoff are now complete on Desktop.

#### ✨ New Features

**Receiving and viewing groups (Phase 1)**

- **`groupConvo` rendering** — Group name, member count, lock status, and a group default avatar appear alongside existing 1:1 DMs in the conversation list. Last-message previews distinguish between `MessageView` / `DeletedMessageView` / `SystemMessageView`.
- **System message timeline** — A new `SystemMessage.tsx` renders all 12 system message variants (member join / leave / added / removed, lock / unlock / permanent lock, group edit, join link create / edit / enable / disable). Member DIDs resolve against `convo.members`, with a short DID fallback when unresolved.
- **Group join link card** — A new `JoinLinkEmbed.tsx` renders `chat.bsky.embed.joinLink#view` with separate states for active / disabled / invalid.
...
- **Lock-aware composer** — Locked groups hide the message input and display a notice. `locked-permanently` shows distinct copy.
...
- **`JoinLinkView` preview screen** — Mounted at `/chat/:code`. Calls `getJoinLinkPreviews` and displays group name, member count, owner, join CTA, and a live status banner.
- **`requestJoin` flow** — On `status: joined` the user is taken directly into the new conversation; on `status: pending` a "request sent" banner appears. Six errors (`ConvoLocked`, `FollowRequired`, `InvalidCode`, `LinkDisabled`, `MemberLimitReached`, `UserKicked`) are mapped to localized messages.
- **`withdrawJoinRequest`** — Pending requests can be withdrawn from the preview screen.

**Owner operations (Phase 3)**

- **New group creation modal** — Group name (≤50 graphemes) plus member selection (≤49, powered by `searchActorsTypeahead`). Seven `createGroup` error conditions are localized.
...
- **Send-join-link hook** — `useSendJoinLinkMessage` is available; the chat picker UI is not yet implemented, so sharing is currently done by pasting the invite URL as plain text (still resolves in-app).
...
- Five new keys for the Chat privacy section (`settings.chatPrivacy`, `settings.allowGroupInvites`, `settings.allowGroupInvitesOption.{all,following,none}`, `settings.allowGroupInvitesHint`, `settings.allowGroupInvitesError`) across de / en / es / fr / id / ja / ko / pt / ru / zh-CN / zh-TW. Earlier phases also added `messages.joinLink.*` keys in ja / en.
...
- **CI/CD** — The macOS Tauri build matrix was removed from `.github/workflows/release.yml`. GitHub Actions now builds Windows binaries only; macOS users should install the [Mac App Store version](https://apps.apple.com/us/app/kazahana/id6761288069) (Catalyst build from `kazahana-ios`).
...
- チャットプライバシーセクション用の新規キー 5 種（`settings.chatPrivacy` / `settings.allowGroupInvites` / `settings.allowGroupInvitesOption.{all,following,none}` / `settings.allowGroupInvitesHint` / `settings.allowGroupInvitesError`）を全 11 言語（de / en / es / fr / id / ja / ko / pt / ru / zh-CN / zh-TW）に追加。先行する Phase で `messages.joinLink.*` キーも ja / en に追加済みです。

#### 🔧 メンテナンス

- **`@atproto/api`** を `^0.20.11` → `^0.20.14` にアップデート。`chat.bsky.group.*` 17 endpoint の codegen、`chat.bsky.embed.joinLink`、`groupConvo`、`systemMessageView` 等の新規型をすべて取り込み。
- **CI/CD** — `.github/workflows/release.yml` から macOS Tauri ビルドマトリクスを撤去。今後 GitHub Actions は Windows バイナリのみをビルドします。macOS 利用者は [Mac App Store 版](https://apps.apple.com/us/app/kazahana/id6761288069)（`kazahana-ios` Catalyst ビルド）をご利用ください。
