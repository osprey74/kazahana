...
- [ ] リポスト経由のいいね/リポスト通知で元ポストが表示されない問題の修正 — 他者のpostをいいね/リポスト後、フォロワーが同様のアクションをした際に「投稿が見つかりません」と表示される（リポスト元ポストのURI解決が必要）
...
- [ ] 公式ピン留め以外のフィードも表示できるようにする
- [x] 画像クリック時の表示方法選択オプション — 設定画面に「アプリ内で表示」/「ブラウザで開く」ラジオボタン追加（11言語対応）
- [ ] リストやフィードの並べ替えをドラッグ操作に対応する（現在は上下ボタン方式）
