...
- [ ] 画像添付の圧縮ロジック改善 — 全入力経路（ペースト/ドラッグ＆ドロップ/ファイル選択）で圧縮を統一、圧縮中インジケータ表示、エラーメッセージ追加
- [ ] 画像編集機能（回転） — 添付画像のプレビューに編集ボタン追加、90度単位の左右回転をCanvas APIで実装、全11言語対応
- [ ] 画像編集機能（クロップ） — 元の比率/正方形/自由比率の3モードでクロップ、ドラッグ操作による選択領域指定、全11言語対応
