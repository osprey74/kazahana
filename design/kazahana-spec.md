...
| 画像 | 最大10枚/投稿, 2MB/枚, JPEG/PNG/WebP（≤4 枚は `app.bsky.embed.images`、≥5 枚は `app.bsky.embed.gallery` で自動切替） |
| 動画 | 最大1本/投稿, 300MB/本, MP4/WebM/MPEG/MOV（lexicon は 100MB / サーバ側でトランスコード） |
| Blob上限 | 50MB |
...
| 画像カルーセル表示 | 5 枚以上の投稿（`app.bsky.embed.gallery`）を横スクロールカルーセル + 枚数バッジで表示、4 枚以下は既存グリッドを維持 | ✅ |
...
| 画像添付 | 最大10枚、プレビュー表示、Altテキストダイアログ（画像プレビュー付き）、5 枚以上選択時に embed 種別を `app.bsky.embed.gallery` へ自動昇格 / 4 枚以下で `app.bsky.embed.images` へ降格 | ✅ |
| 画像ドラッグ&ドロップ / ペースト | ドラッグ&ドロップ・クリップボードペースト対応、自動圧縮（2MB以内、4000px²から×0.8 反復） | ✅ |
| 画像編集 | 90°回転、クロップ（フリー/オリジナル/正方形アスペクト比） | ✅ |
| 動画添付 | 最大300MB（サーバ受容範囲）、MP4/WebM/MPEG/MOV、トランスコード進捗表示、Altテキスト対応 | ✅ |
| アップロード進捗オーバーレイ | 投稿モーダルに半透明オーバーレイ + 大型スピナー + 進捗バー、画像（n/m）・動画（%）の状態別表示 | ✅ |
